...
from sensor_sim.uwb.rmse_interpolation import RMSESplineInterpolator
from sensor_sim.uwb.sigma_interpolation import SigmaSplineInterpolator
...
class UWBSimulator():
    ...
    def _read_config(self):
        ...
        self.topic_prefix = rospy.get_param('~topic_prefix', '/uwb')

    def _create_publishers(self):
        self.publishers = []

        for anchor in self.anchors:
            rospy.loginfo('Will publish to {}'.format(self.topic_prefix + '/tag/' + self.tag + '/anchor/' + anchor))
            pub = rospy.Publisher(self.topic_prefix + '/tag/' + self.tag + '/anchor/' + anchor, UWBRange, queue_size=10)
